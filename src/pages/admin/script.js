...
// ==========================
window.viewPhotos = function (images) {
...
async function loadLocationsList() {
    ...
    try {
        ...
        let html = '<button class="btn-create" onclick="openLocationForm()">+ Criar Novo Local</button>';
        ...
        locations.forEach(location => {
            ...
            if (Array.isArray(location.images)) {
                ...
                images = location.images.split(',').map(img => img.trim()).filter(img => img);
            }

            const firstImage = images.length > 0 ? images[0] : '';
            const imageHtml = firstImage ? `<img src="${firstImage}" alt="${location.name}" class="location-grid-img">` : '<div class="location-grid-no-img">📍</div>';
        ...
        container.innerHTML = '<p style="color: red;">Erro ao carregar locais. Tente novamente.</p>';
    }
}
...
async function viewLocationDetails(locationId) {
    try {
        ...
        if (location.accessibility_items && location.accessibility_items.length > 0) {
            accessibilityHtml = location.accessibility_items.map(item => `
                <li>
                    ${item.image ? `<img src="${item.image}" alt="${item.name}" style="width: 20px; height: 20px; margin-right: 8px;">` : ''}
                    ${item.name}
                </li>
            `).join('');
        } else {
            ...
            <div class="modal-view-location">
                ...
                <section class="detail-info-section">
                    <div class="detail-header">
                        <h1>${location.name}</h1>
                        <button class="btn-edit-main" onclick="openLocationForm(${location.id})">Editar Local</button>
                    </div>
...
async function openLocationForm(locationId = null) {
    ...
    if (locationId !== null) {
        ...
        try {
            ...
            container.innerHTML = '<p style="color: red;">Erro ao carregar dados do local.</p>';
            return;
        }
    }
    ...
    try {
        ...
        accessibilityItems = Array.isArray(response) ? response : (response?.accessibility_items || []);
    } catch (error) {
    ...
    let html = `
        ...
        <form class="location-form" id="locationForm">
            ...
            <div class="form-group">
                ...
                <textarea id="locDescription" name="description">${location?.description || ''}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="locImages" name="images">${location?.images ? location.images.join(', ') : ''}</textarea>
            </div>
                    ...
                    ${accessibilityItems.map(item => `
                        <div class="accessibility-item-card">
                            <input type="checkbox" id="acc-${item.id}" value="${item.id}" 
                                ${selectedAccessibilityItems.includes(item.id) ? 'checked' : ''} />
                            <div class="accessibility-item-content">
                                ${item.image ? `<img src="${item.image}" alt="${item.name}" class="accessibility-icon">` : ''}
                                <label for="acc-${item.id}">${item.name}</label>
                            </div>
                        </div>
                    `).join('')}
                    ${accessibilityItems.length === 0 ? '<p style="color: #999; margin: 0;">Nenhum item disponível</p>' : ''}
                </div>
            </div>
    ...
    document.getElementById('locationForm').addEventListener('submit', async (e) => {
        ...
        if (imagesStr) {
            data.images = imagesStr.split(',').map(url => url.trim()).filter(url => url);
        } else {
            data.images = [];
        }

        // Coletar IDs de acessibilidade selecionados
        const selectedCheckboxes = document.querySelectorAll('.accessibility-items input[type="checkbox"]:checked');
        data.accessibility_items = Array.from(selectedCheckboxes).map(cb => parseInt(cb.value));
